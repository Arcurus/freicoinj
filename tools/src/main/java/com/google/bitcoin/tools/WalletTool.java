...
public class WalletTool {
  ...
  private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) throws VerificationException {
    try {
      ...
      t.setRefHeight(chain.getChainHead().getHeight());
      ...
      for (String spec : outputs) {
        ...
        try {
          ...
          fee = Transaction.getDemurrageInSatoshi(chain.getChainHead().getHeight()-1000, chain.getChainHead().getHeight(),new BigDecimal(parts[1]));
  ...
  private static void addKey() {
    ...
    if (options.has("privkey")) {
      ...
      if (data.startsWith("L") || data.startsWith("5H") || data.startsWith("5K")) {

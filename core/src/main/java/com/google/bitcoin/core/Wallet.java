...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
  ...
  public void completeTx(SendRequest req) throws InsufficientMoneyException {
    ...
    try {
      ...
      BigInteger feeval = BigInteger.ZERO;
      for (Transaction tx : unspent.values()) { 
        int oldheight = (int)tx.getRefHeight();
        BigDecimal val = (new BigDecimal(tx.getValueSentToMe(this))).movePointLeft(8);
        feeval = feeval.add(Transaction.getDemurrageInSatoshi(oldheight-2,height,val));
      }  
      //if (feeval.compareTo(BigInteger.valueOf(10000)) == -1) feeval = feeval.add(BigInteger.valueOf(10000));
      Transaction.REFERENCE_DEFAULT_MIN_TX_FEE = feeval;
      req.DEFAULT_FEE_PER_KB = feeval;
      req.feePerKb = feeval;

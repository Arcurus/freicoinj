...
public class BitcoinSerializer {
  ...
  int arrtoint(byte[] bytes,int i) {
    return bytes[i+3] << 24 | (bytes[i+2] & 0xFF) << 16 | (bytes[i+1] & 0xFF) << 8 | (bytes[i] & 0xFF);
  }

  int arrtoshort(byte[] bytes,int i) {
  ...
  public Message deserializePayload(BitcoinPacketHeader header, ByteBuffer in) throws ProtocolException, BufferUnderflowException {
    ...
    try {
      ...
      throw new ProtocolException(Utils.bytesToHexString(payloadBytes));
    }
  }

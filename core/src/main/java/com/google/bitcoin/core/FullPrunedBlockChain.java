...
public class FullPrunedBlockChain extends AbstractBlockChain {    
    ...
    protected TransactionOutputChanges connectTransactions(int height, Block block)
        ...
        try {
            ...
            for (final Transaction tx : block.transactions) {
                ...
                if (!isCoinBase) {
                    ...
                    for (int index = 0; index < tx.getInputs().size(); index++) {
                        ...
                        if (prevOut == null) throw new VerificationException("Attempted to spend a non-existent or already spent output!");
            ...
            //if (totalFees.compareTo(params.MAX_MONEY) > 0 || block.getBlockInflation(height).add(totalFees).compareTo(coinbaseValue) < 0) throw new VerificationException("Transaction fees out of range");
